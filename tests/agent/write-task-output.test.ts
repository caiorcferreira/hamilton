...
import { validateAndWrite } from "../../src/agent/write-task-output.js"
...
describe("validateAndWrite", () => {
  ...
  it("writes output JSON to file and returns success", () => {
    const result = validateAndWrite("run-1", "task-1", undefined, { status: "done", key: "val" })
  ...
  it("returns error on duplicate write", () => {
    validateAndWrite("run-1", "task-1", undefined, { status: "done" })
    const result = validateAndWrite("run-1", "task-1", undefined, { status: "done" })
  ...
  it("returns error when input is an array", () => {
    const result = validateAndWrite("run-1", "task-1", undefined, [1, 2, 3])
    expect(result.success).toBe(false)
    if (!result.success) {
  ...
  it("returns error when input is null", () => {
    const result = validateAndWrite("run-1", "task-1", undefined, null)
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error).toContain("JSON object")
    }
  ...
  it("returns error when status field is missing", () => {
    const result = validateAndWrite("run-1", "task-1", undefined, { repo: "hamilton" })
    expect(result.success).toBe(false)
    if (!result.success) {
  ...
  it("returns error when status is empty", () => {
    const result = validateAndWrite("run-1", "task-1", undefined, { status: "" })
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error).toContain("Missing required field 'status'")
    }
  ...
  it("validates with schema and rejects invalid output", () => {
    ...
    const result = validateAndWrite("run-1", "task-1", schema, { status: "done", count: "not-a-number" })
    expect(result.success).toBe(false)
    if (!result.success) {
  ...
  it("validates with schema and accepts valid output", () => {
    ...
    const result = validateAndWrite("run-1", "task-1", schema, { status: "done", count: 42 })
    expect(result.success).toBe(true)
  })

  it("skips schema validation when schema is undefined", () => {
    const result = validateAndWrite("run-1", "task-1", undefined, { status: "done", anyField: "anyValue" })
    expect(result.success).toBe(true)
  })
})
