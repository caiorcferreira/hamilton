    ...
    const result = await Effect.runPromiseExit(
      Effect.scoped(
        Effect.gen(function* (_) {
          ...
          return yield* _(runWorkflow(testSpec, { user_input: "test", project_dir: tmpHome }, { strict: false }, [], null))
        })
    ...
    const result = await Effect.runPromiseExit(
      Effect.scoped(
        Effect.gen(function* (_) {
          ...
          yield* _(Effect.sleep("10 millis"))
          return yield* _(runWorkflow(testSpec, { user_input: "build a login page", project_dir: tmpHome }, { strict: false }, [], null))
        })
      ).pipe(Effect.provide(EventBusLive))
    )
  ...
  it("writes PromptBuilt event to task logs via TaskLogger", async () => {
    const result = await Effect.runPromiseExit(
      Effect.scoped(
        Effect.gen(function* () {
          yield* TaskLogger
          return yield* runWorkflow(testSpec, { user_input: "test", project_dir: tmpHome }, { strict: false }, [], null)
        })
      ).pipe(Effect.provide(EventBusLive))
    )
