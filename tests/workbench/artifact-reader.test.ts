...
import { createArtifactReader, type ArtifactFileSystem, type ArtifactReadResult } from "../../src/workbench/artifact-reader.js"
...
describe("artifact reader", () => {
  it("reads recognized frontmatter and preserves multiline body locations", async () => {
    const result = await createArtifactReader(readFrom("---\nartifact: proposal\nchange: demo\n---\n# Title\n\nBody\n"))("changes/demo/proposal.md")
  ...
  it("reads the first frontmatter block", async () => {
    ...
    if (result._tag === "recognized") expect(result.metadata.artifact).toBe("proposal")
  })
  ...
  it("classifies files without frontmatter as unrelated", async () => {
    const result = await createArtifactReader(readFrom("# Not an artifact\n\nText\n"))("notes.md")
  ...
  it("reports read failures through a typed diagnostic", async () => {
    ...
    if (result._tag === "invalid") expect(result.diagnostic.message).toContain("permission denied")
  })

  it("supports asynchronous injected file access", async () => {
    const result = await createArtifactReader({ readFile: async () => "---\nartifact: plan\n---\nbody" })("plan.md")
