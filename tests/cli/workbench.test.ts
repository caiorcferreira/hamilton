...
describe("workbench CLI", () => {
  ...
  beforeEach(() => {
    temporaryDirectory = Fs.mkdtempSync(Path.join(Os.tmpdir(), "hamilton-workbench-cli-"))
  })
  ...
  it("lints an unrelated file successfully", () => {
    ...
    const result = runCli(temporaryDirectory, "workbench", "lint", "--file", file)
  ...
  it("rejects invalid lint scopes before inspecting files", () => {
    ...
    const both = runCli(temporaryDirectory, "workbench", "lint", "--file", file, "--change-dir", temporaryDirectory)
    const unknown = runCli(temporaryDirectory, "workbench", "lint", "--unknown")
  ...
  it("preserves isolate negative checks", () => {
    ...
    const result = runCli(temporaryDirectory, "workbench", "isolate", "--check")
  ...
  it("preserves prototype verification failures", () => {
    ...
    const result = runCli(temporaryDirectory, "workbench", "prototype", "--verify", "prototype/expected")
  ...
  it("packages a whole-change diff", () => {
    ...
    const result = runCli(temporaryDirectory, "workbench", "diff", "--whole-change")

    expect(result.status).toBe(0)
    expect(result.stdout).toContain("range:")
    expect(result.stdout.trimEnd()).toMatch(/\/tmp\/hamilton-diff-whole-change-[^\n]+$/)
  })
  ...
  it("runs precondition gates and reports a closed gate", () => {
    ...
    git(temporaryDirectory, "config", "user.email", "test@example.com")
    git(temporaryDirectory, "config", "user.name", "Test")
    const changeDirectory = Path.join(temporaryDirectory, ".hamilton", "changes", "sample")
  ...
  it("reports failed operations without helper scripts", () => {
    const result = runCli(temporaryDirectory, "workbench", "context", Path.join(temporaryDirectory, "missing"))
