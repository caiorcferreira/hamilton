...
const referenceUrl = (name: string) =>
  new URL(
    `../../skills/hamilton-orchestrate/references/${name}`,
    import.meta.url,
  );
...
describe("hamilton-orchestrate whole-branch resume contract", () => {
  ...
  it("dispatches whole-branch review when review is malformed or stale", () => {
    expect(matrix).toMatch(
      /\| stale or malformed \| Dispatch `hamilton-review`/,
    );
  });
...
describe("hamilton-orchestrate checkpoint and evidence contract", () => {
  ...
  it("orders the stable checkpoint, code, feedback, and next task", () => {
    const checkpoint = process.indexOf(
      "Resolve and validate the task checkpoint",
    );
  ...
  it("reconstructs or stops instead of rebasing historical work", () => {
    ...
    expect(checkpointRules).toMatch(
      /every available candidate.*same full commit/i,
    );
    expect(checkpointRules).toMatch(
    ...
    expect(checkpointRules).toMatch(
      /never.*current `HEAD`.*historical evidence/is,
    );
  });
  ...
  it("uses task progress as the sole detailed implementer report", () => {
    expect(skill).toMatch(
      /task progress is the only detailed implementer report/i,
    );
    expect(implementer).toContain("<change-dir>/tasks/task-N/progress.md");
    expect(implementer).toMatch(/return only.*status.*commit/is);
    expect(
      [skill, implementer, codeFeedback, wholeBranch].join("\n"),
    ).not.toContain("[REPORT_FILE]");
  });
  ...
  it("defines one exact durable task approval predicate", () => {
    ...
    expect(approval).toMatch(
      /tracked at current `HEAD`.*`git ls-files --error-unmatch`/is,
    );
    expect(approval).toMatch(
    ...
    expect(approval).toMatch(
      /latest commit that touched.*feedback.*artifact-only/is,
    );
    expect(approval).toMatch(
      /commit's path list.*only.*tasks\/task-N\/feedback\.md/is,
    );
    expect(approval).toMatch(
      /physical last pass.*valid.*`approved`.*no blocking findings.*fresh/is,
    );
    expect(approval).toMatch(
      /any failed condition.*dispatch `hamilton-code-feedback`/is,
    );
  });

  it("rechecks durable approval at every advancement boundary", () => {
    expect(process).toMatch(
      /Load durable state.*evaluate.*durable task approval/is,
    );
    expect(process).toMatch(
    ...
    expect(process).toMatch(
      /never.*transient.*subagent output.*worktree text.*advance/is,
    );
  });
});
...
describe("hamilton-orchestrate prompt scopes", () => {
  ...
  it("ships distinct task-feedback and whole-branch prompt files", () => {
    ...
    expect(Fs.existsSync(referenceUrl("whole-branch-review-prompt.md"))).toBe(
      true,
    );
...
describe("hamilton-orchestrate whole-branch findings", () => {
  ...
  it("re-plans implementation findings as numbered remediation tasks", () => {
    expect(findings).toMatch(
      /complete finding set.*`hamilton-plan` in re-plan mode/is,
    );
    expect(findings).toMatch(/one or more.*numbered remediation tasks/is);
    expect(findings).toMatch(
      /each.*ordinary `hamilton-code`.*`hamilton-code-feedback` loop/is,
    );
  ...
  it("forbids ownerless, retroactive, and synthetic fixes", () => {
    ...
    expect(findings).toMatch(
      /every correction.*numbered active task present in `plan\.md`/i,
    );
  });
});
