...
describe("hamilton-finish-work contract", () => {
  it("consumes the exact split-pipeline evidence package", () => {
    ...
    expect(inputs).toMatch(
      /physically last.*attempt.*feedback pass.*whole-branch.*pass/is,
    );
  ...
  it("fails closed on every exact ledger, feedback, and review gate", () => {
    ...
    expect(preconditions).toMatch(
      /physically last\s+task\s+attempt.*(?:`done`|`Outcome: done`)/is,
    );
    expect(preconditions).toMatch(
      /physically last task feedback pass.*valid.*fresh.*`approved`/is,
    );
    expect(preconditions).toMatch(
      /physically last whole-branch review pass.*valid.*`approved`/is,
    );
    ...
    expect(preconditions).toMatch(
      /do not.*(?:create|change|write).*`finish\.md`/is,
    );
  });
  ...
  it("limits the explicit waiver to material-change ancestry", () => {
    ...
    expect(preconditions).toMatch(
      /does not waive.*ledger.*task feedback.*whole-branch.*validity.*verdict.*blocking/is,
    );
  });
  ...
  it("instantiates the installed finish template and preserves finish history", () => {
    ...
    expect(history).not.toMatch(
      /```(?:markdown)?[\s\S]*?Passed preconditions:[\s\S]*?Route intent:[\s\S]*?```/,
    );
    ...
    expect(skill).not.toMatch(
      /append(?:s|ing)? (?:a )?finish (?:entry|section) to `?progress\.md`?/i,
    );
  });
  ...
  it("commits the attempt after gates and spec synchronization but before effects", () => {
    ...
    expect(process).toMatch(
      /preconditions.*synchroniz.*specification.*commit/is,
    );
  ...
  it("reconciles a dangling attempt before allocating or executing another", () => {
    ...
    expect(recovery).toMatch(
      /inspect.*git.*remote.*pull\s+request.*route.*workspace/is,
    );
    expect(recovery).toMatch(
      /append.*matching.*Outcome N.*continue.*same attempt/is,
    );
  ...
  it("allows only finish-owned post-gate mutations for the same attempt", () => {
    ...
    expect(boundary).toMatch(
      /canonical.*specs.*route.*map.*numbered.*finish/is,
    );
  ...
  it("routes missing or incorrect change requirements out of finish-work", () => {
    ...
    expect(synchronization).toMatch(
      /canonical.*\.hamilton\/specs\/.*already approved.*artifacts/is,
    );
    expect(synchronization).toMatch(
      /missing or incorrect.*change requirement.*abort/is,
    );
    expect(synchronization).toMatch(
      /artifact revision.*fresh whole-branch review/is,
    );
    expect(synchronization).toMatch(
      /never.*(?:add|edit|rewrite).*requirements\//is,
    );
    expect(boundary).toMatch(
      /only.*canonical.*\.hamilton\/specs\/.*route.*map.*finish\.md/is,
    );
  ...
  it("persists a verified local-merge outcome on the base branch", () => {
    ...
    expect(strategies).toMatch(
      /local-merge:.*merge.*attempt\s+commit.*base branch/is,
    );
    expect(strategies).toMatch(/local-merge:[\s\S]*remove.*worktree/is);
    expect(strategies).toMatch(
      /local-merge:[\s\S]*read back.*base.*workspace/is,
    );
    expect(strategies).toMatch(
      /local-merge:[\s\S]*matching `Outcome N`.*base branch.*commit/is,
    );
  });
  ...
  it("pushes and reads back a pull-request outcome", () => {
    ...
    expect(strategies).toMatch(
      /pull-request:[\s\S]*push.*open.*(?:pull|merge) request/is,
    );
    expect(strategies).toMatch(
      /pull-request:[\s\S]*read back.*URL.*head.*base/is,
    );
    expect(strategies).toMatch(
      /pull-request:[\s\S]*matching `Outcome N`.*commit.*push/is,
    );
    expect(strategies).toMatch(
      /pull-request:[\s\S]*verify.*request.*outcome\s+commit/is,
    );
  });
  ...
  it("verifies and persists no-op without moving the workspace", () => {
    ...
    expect(strategies).toMatch(
      /no-op:[\s\S]*verify.*branch.*worktree.*remain/is,
    );
    expect(strategies).toMatch(
      /no-op:[\s\S]*matching `Outcome N`.*change branch.*commit/is,
    );
  });
  ...
  it("reports only verified effects and persisted history", () => {
    ...
    expect(output).toMatch(
      /never claim.*merge.*pull request.*no-op.*route.*workspace.*outcome/is,
    );
