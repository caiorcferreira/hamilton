...
Code starts one identified task by moving only its matching frontmatter metadata entry and row together to `in-progress`; the attempt-free task log remains `status: pending` with no attempt, and its local status never becomes `in-progress` before an attempt exists. A previously finalized task log retains its latest `done` or `blocked` local status while a correction reopens the root task row. A completed implementation appends the task-local attempt, sets that log's frontmatter status to the matching `done` or `blocked` outcome, and changes the same root metadata entry and row together to that outcome; a correction can reopen a prior `done` or `blocked` task. Sibling metadata entries, rows, and histories remain unchanged. An interrupted run leaves `in-progress` as an explicit signal for inspection. A blocked run persists its task evidence and status without committing partial production edits.
