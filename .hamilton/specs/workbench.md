...
- Fully evidenced `Base`, `Head`, and `Verdict` fields MUST be parsed as pass-local feedback or review evidence, while legacy-global provenance applies only to the physically last legacy pass; the physically latest evidenced pass MUST be shared by lint, context, and precondition.
