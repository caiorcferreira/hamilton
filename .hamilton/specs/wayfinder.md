...
- The skill MUST NEVER resolve more than one ticket per session, with the exception of research tickets. The budget is a ceiling: a claimed ticket MUST NEVER be parked — claiming is never a handoff, and the claiming session MUST take the ticket as far as its type allows (a HITL ticket to resolution; a research ticket to dispatch, resolving when its findings return and step 1 absorbs them).
