...
- a ticket is claimed -> the same session resolves it; resolution is never deferred to a later session
...
- The skill MUST NEVER resolve more than one ticket per session, with the exception of research tickets. The budget is a ceiling: a claimed ticket MUST be resolved in the claiming session, NEVER deferred to a later one.
