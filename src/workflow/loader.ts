...
export function loadWorkflowSpec(
  ...
  return Effect.gen(function* (_) {
    const dir = Path.join(workflowsDir, workflowSlug)
    const filePath = Path.join(dir, "workflow.yml")

    const content = yield* _(
      Effect.try({
        try: () => Fs.readFileSync(filePath, "utf-8"),
        catch: () => new WorkflowNotFoundError({ workflowSlug, dir })
      })
    )
    ...
    const raw = yield* _(
      Effect.try({
        ...
        catch: (e) => new WorkflowParseError({ workflowSlug, message: String(e) })
      })
    )
