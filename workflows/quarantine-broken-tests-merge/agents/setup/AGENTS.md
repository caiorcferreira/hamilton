...
Each key must use the EXACT names above. Downstream steps reference these as `{{tasks.setup.outputs.original_branch}}`, `{{tasks.setup.outputs.build_cmd}}`, `{{tasks.setup.outputs.test_cmd}}`, `{{ci_notes}}`, `{{baseline}}`. Do not nest values inside other keys.
