...
- [ ] Establish an isolated workspace for this unit off `port-wayfinder-siblings`: create a worktree on a dedicated branch (e.g. `unit-10-map-mechanics-contract`) and `cd` into it, confirming `git rev-parse --show-toplevel` ends in that worktree.
